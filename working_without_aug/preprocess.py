...
def generate_spectrogram(audio, n_fft=2048, hop_length=512, target_length=128):
    ...
    try:
        spectrogram = librosa.stft(audio, n_fft=n_fft, hop_length=hop_length)
        spectrogram_db = librosa.amplitude_to_db(np.abs(spectrogram))
        spectrogram_db = np.pad(spectrogram_db, ((0, 0), (0, max(0, target_length - spectrogram_db.shape[1]))), mode="reflect")[:, :target_length]
        spectrogram_db = (spectrogram_db - spectrogram_db.min()) / (spectrogram_db.max() - spectrogram_db.min())  # Normalize
        return spectrogram_db
    except Exception as e:
        logging.error(f"Error generating spectrogram: {e}")
        return None
...
def preprocess_and_save_all(input_dir, output_dir, plot_emotion=None, sr=16000, target_length=128):
    ...
    plot_done = False
    processed_count = 0  # Track processed files
...
def preprocess_and_save_all(input_dir, output_dir, plot_emotion=None, sr=16000, target_length=128):
    ...
    logging.info(f"Sampling rate: {sr}, Target spectrogram length: {target_length}")
    ...
    for file_path in input_path.rglob("*.wav"):
        ...
        try:
            ...
            if spectrogram is not None:
                processed_count += 1
                logging.info(f"Spectrogram saved: {output_file_path}")
            else:
                skipped_count += 1
                logging.warning(f"Skipping file: {file_path}. Spectrogram generation failed.")
            ...
            if not plot_done and plot_emotion and plot_emotion.lower() in file_path.parts and spectrogram is not None:
                ...
                if audio is not None:
                    plot_waveform_and_spectrogram(audio, spectrogram, sr=sr, title=f"{plot_emotion.capitalize()} Example")
                    plot_done = True  # Ensure only one plot is generated
                    logging.info(f"Generated plot for emotion: {plot_emotion}")
    ...
    # Final summary
    logging.info(f"Preprocessing complete. Total processed files: {processed_count}, Total skipped files: {skipped_count}")
